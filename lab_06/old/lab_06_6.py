...
def run_single_experiment(image_path: str, n_processes: int, max_iterations: int,
                          checkpoint_step: int = 10000, verbose: bool = True):
    """
    ...
    if n_processes == 1:
        ...
        step_func = algo.step
        # Для последовательной версии используем меньший батч
        batch_size = 1000
    else:
        algo = OptimizedParallelDPD(image_path, n_agents=100, n_processes=n_processes)
        step_func = algo.parallel_step
        # Для параллельной версии увеличиваем батч для амортизации overhead
        batch_size = 5000

    # Начальный чекпоинт
    algo.save_checkpoint(0, save_to_history=(checkpoint_step <= 10000))
    ...
    while current_iteration < max_iterations:
        ...
        if current_iteration % checkpoint_step == 0 or current_iteration == max_iterations:
            save_to_history = (checkpoint_step >= 50000 or current_iteration <= 10000)
            algo.save_checkpoint(current_iteration, save_to_history=save_to_history)
...
def plot_scalability(results: dict, iterations: int):
    ...
    ideal_speedups = [n for n in n_processes]

    # Создаем фигуру с двумя графиками
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

    # График 1: Время выполнения
    ax1.plot(n_processes, times, 'bo-', linewidth=2, markersize=8, label='Реальное время')
    ax1.plot(n_processes, ideal_times, 'r--', linewidth=2, label='Идеальное время (T₁/M)')
    ax1.set_xlabel('Количество процессов', fontsize=12)
    ax1.set_ylabel('Время выполнения (секунды)', fontsize=12)
    ax1.set_title(f'Зависимость времени от числа процессов\n({iterations:,} итераций)', fontsize=14)
    ...
    # График 2: Ускорение
    ax2.plot(n_processes, speedups, 'go-', linewidth=2, markersize=8, label='Реальное ускорение')
    ax2.plot(n_processes, ideal_speedups, 'r--', linewidth=2, label='Идеальное ускорение (линейное)')
    ax2.set_xlabel('Количество процессов', fontsize=12)
    ax2.set_ylabel('Ускорение (S = T₁/Tₘ)', fontsize=12)
    ax2.set_title('Ускорение относительно однопоточной версии', fontsize=14)
    ax2.grid(True, alpha=0.3)
    ax2.legend(fontsize=11)

    # Добавляем значения на график
    for i, (n, s) in enumerate(zip(n_processes, speedups)):
        if i % 3 == 0 or n == n_processes[-1]:
...
def plot_convergence(metric_history: list):
    ...
    ax.plot(iterations, metrics, 'b-', linewidth=2, marker='o', markersize=6)
    ax.set_xlabel('Итерации', fontsize=12)
    ax.set_ylabel('Метрика ошибки', fontsize=12)
    ax.set_title('Сходимость алгоритма: зависимость метрики от итераций', fontsize=14)
    ...
    key_points = [0, len(iterations) // 4, len(iterations) // 2, 3 * len(iterations) // 4, len(iterations) - 1]
...
if __name__ == "__main__":
    ...
    print("\n3. Рекомендации:")
    if best_speedup > 2.0:
        print(f"   ✓ Параллелизация эффективна: лучшее ускорение {best_speedup:.2f}x на {best_n_proc} процессах")
    else:
        print(f"   ⚠ Параллелизация малоэффективна: максимальное ускорение {best_speedup:.2f}x")

    if efficiency > 50:
        print(f"   ✓ Использование 24 процессов эффективно (эффективность {efficiency:.1f}%)")
    elif efficiency > 25:
        print(f"   ~ Использование 24 процессов приемлемо (эффективность {efficiency:.1f}%)")
    else:
        print(f"   ✗ Рекомендуется использовать меньше процессов (эффективность {efficiency:.1f}%)")

    if convergence_rates[-1] < convergence_rates[0] * 0.1:
        print(f"   ⚠ Скорость сходимости значительно замедляется на поздних итерациях")
