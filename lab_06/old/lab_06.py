...
class DynamicsWithPreferredDistribution:
    ...
    def __init__(self, image_path: str, n_agents: int = 100, seed: int = 42):
        ...
        self.prepare_random_orders(1000)
    ...
    def load_image(self, image_path: str):
        ...
        # Инвертируем (чтобы темные области имели большие значения)
        self.target_distribution = 255 - img_array

        # Добавляем небольшое значение везде (чтобы избежать нулей)
        self.target_distribution += 1.0
    ...
    def prepare_random_orders(self, n_orders: int = 1000):
        """Подготовка случайных порядков проверки направлений"""
    ...
    def calculate_metric(self) -> float:
        ...
        # Вычисляем MSE
        diff = self.target_distribution - normalized_dynamic
        mse = np.mean(diff ** 2)
    ...
    def get_current_image(self) -> np.ndarray:
        ...
        # Инвертируем обратно для визуализации
    ...
    def save_checkpoint(self, iteration: int):
        ...
        print(f"Итерация {iteration:,}: метрика = {metric:.2f}, M = {self.M:,}")
    ...
    def visualize_progress(self, save_path: str = 'progress.png'):
        ...
        # Показываем оригинал
        axes[0].imshow(255 - self.target_distribution, cmap='gray', vmin=0, vmax=255)
...
def worker_task(args):
    ...
    local_dynamic = dynamic_dist.copy()
    local_M = M
            ...
            for dir_idx in order:
                ...
                if 0 <= new_x < width and 0 <= new_y < height:
                    ...
                    if local_M == 0:
                        ...
                        normalized_dynamic = (N_target / local_M) * local_dynamic[new_y, new_x]
...
class ParallelDPD(DynamicsWithPreferredDistribution):
    ...
    def parallel_step(self, n_steps: int = 1):
        ...
        # Сначала сбрасываем динамическое распределение и M
        self.dynamic_distribution = np.zeros_like(self.dynamic_distribution)
        self.M = 0

        # Обрабатываем результаты каждого воркера
...
def run_experiment(image_path: str, use_parallel: bool = True, n_processes: int = None, max_iterations: int = 5000000):
    """
    Запуск эксперимента
    ...
    print(f"\nНачальная метрика: {initial_metric:.2f}")
    ...
    for checkpoint in checkpoints:
        ...
        for batch in range(n_batches):
            ...
            if (batch + 1) % 10 == 0:
                ...
                if elapsed > 0:
                    current_steps = (batch + 1) * batch_size
                    speed = current_steps / elapsed
                    print(f"  Прогресс: {current_iteration + current_steps:,}/{checkpoint:,} ({speed:.0f} шагов/сек)",
                          end='\r')
    ...
    print(f"Финальная метрика: {final_metric:.2f}")
...
def compare_performance(image_path: str):
    ...
    test_processes = [2, 4, 8, available_cores]
    ...
    for name, time_val in results.items():
        ...
        print(f"{name:20s}: {time_val:7.2f} сек (ускорение: {speedup:.2f}x)")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(image_path):
        ...
        # Рисуем круг
        draw.ellipse([50, 50, 150, 150], fill=0, outline=0)
        # Рисуем прямоугольник
        draw.rectangle([75, 75, 125, 125], fill=128)
        img.save(image_path)
        print(f"Тестовое изображение сохранено в {image_path}")

    # Запуск одного эксперимента (быстрее)
    print("\nЗапуск основного эксперимента...")
    run_experiment(image_path, use_parallel=True, max_iterations=1000000)
