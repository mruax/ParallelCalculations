...
if __name__ == "__main__":
    ...
    for count in range(4):
        ...
        # Настройка шрифтов
        plt.rcParams.update({'font.size': 20, 'font.weight': 'bold'})  # основной шрифт жирный 20
        plt.rcParams['axes.labelweight'] = 'bold'

        # Создаём папку, если её нет
        save_dir = Path("../src/lab_03")
        ...
        ax.tick_params(axis='both', which='major', labelsize=14)  # координатные значения меньше, 14 кегль
        ax.grid(True)
        plt.tight_layout()
        plt.savefig(save_path, dpi=300)
        plt.close()

        print(f"График сохранён по пути: {save_path.resolve()}")
