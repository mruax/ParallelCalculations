...
# Коэффициент трения
MU = 0.1  # Коэффициент вязкого трения
...
def update_particles(particles: List[Particle], dt: float, box_size: float) -> None:
    ...
    """
    for p in particles:
        # Обновление скоростей: v^(n+1) = v^n + (dt/m) * F^n
        p.vx += (dt / MASS) * p.fx
        p.vy += (dt / MASS) * p.fy
        ...
        # Обновление координат: x^(n+1) = x^n + dt * v^(n+1)
        p.x += dt * p.vx
        p.y += dt * p.vy
...
def plot_trajectory_sample(trajectories: np.ndarray, num_particles_to_plot: int = 10,
    ...
    for i in range(min(num_particles_to_plot, NUM_PARTICLES)):
        ...
        # Начальная точка
        ax.scatter(trajectory[0, 0], trajectory[0, 1], s=100, marker='o',
                   color=colors[i], edgecolors='black', linewidths=2, zorder=5)
        # Конечная точка
        ax.scatter(trajectory[-1, 0], trajectory[-1, 1], s=100, marker='s',
                   color=colors[i], edgecolors='black', linewidths=2, zorder=5)
...
def main():
    ...
    plot_trajectory_sample(trajectories, num_particles_to_plot=10)
