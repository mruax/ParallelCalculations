...
def worker_task(args):
    ...
    entry_x, entry_y = shared_entry
    exit_x, exit_y = shared_exit
    ...
    agents_x = []
    agents_y = []
    agent_paths = []  # Сохраняем пути агентов

    # Инициализация агентов на входе
    for _ in agent_indices:
        agents_x.append(entry_x)
        agents_y.append(entry_y)
        agent_paths.append([(entry_x, entry_y)])
        local_dynamic[entry_y, entry_x] += 1
        local_M += 1

    current_order_idx = start_order_idx
    n_orders = len(random_orders)

    exit_found_locally = False
    winning_agent_idx = -1

    # Выполняем итерации
    for iteration in range(n_iterations):
        # Проверяем, не нашел ли кто-то из других процессов выход
        if shared_exit_found.value == 1:
            break

        for agent_idx in range(len(agent_indices)):
            x, y = agents_x[agent_idx], agents_y[agent_idx]

            # Проверяем, достиг ли этот агент выхода
            if x == exit_x and y == exit_y:
                exit_found_locally = True
                winning_agent_idx = agent_idx
                shared_exit_found.value = 1
                break
            ...
            for dir_idx in order:
                ...
                if 0 <= new_x < width and 0 <= new_y < height and maze[new_y, new_x] == 1:
                    # Вычисляем K = alpha / (n + 1)
                    n_visits = local_dynamic[new_y, new_x]
                    alpha_value = target_dist[new_y, new_x]

                    # Если альфа -inf, пропускаем эту клетку (недостижима)
                    if not np.isinf(alpha_value):
                        k = alpha_value / (n_visits + 1)
            ...
            agent_paths[agent_idx].append((best_x, best_y))

            # Обновляем локальное динамическое распределение
            local_dynamic[best_y, best_x] += 1
            local_M += 1

        if exit_found_locally:
            break

    # Возвращаем результат с информацией о пути, если нашли выход
    winning_path = None
    if exit_found_locally and winning_agent_idx >= 0:
        winning_path = agent_paths[winning_agent_idx]

    return local_dynamic, local_M, exit_found_locally, winning_path
...
class MazeSolver:
    ...
    def __init__(self, maze_image_path: str, n_agents: int = 100, seed: int = 42):
        ...
        # Создание эталонного распределения (градиент альфа через BFS)
        self.compute_alpha_metric()
        self.target_distribution = self.alpha  # Используем альфа как эталонное распределение
    ...
    def find_entry_exit(self) -> Tuple[Tuple[int, int], Tuple[int, int]]:
        """
        Находит вход (слева) и выход (справа) в лабиринте
        Returns: ((entry_x, entry_y), (exit_x, exit_y))
        """
        # Ищем вход слева (первая пустая клетка в первом столбце)
        entry = None
        ...
        if entry is None:
            # Если не найден, берем центр
            entry = (0, self.height // 2)

        # Ищем выход справа (первая пустая клетка в последнем столбце)
        exit_point = None
        for y in range(self.height):
    ...
    def run_sequential(self, n_iterations: int) -> Tuple[float, bool]:
        ...
        for _ in range(self.n_agents):
            ...
            agents_y.append(entry_y)
            agent_paths.append([(entry_x, entry_y)])
            self.dynamic_distribution[entry_y, entry_x] += 1
            self.M += 1
        ...
        for iteration in range(n_iterations):
            for agent_idx in range(self.n_agents):
                ...
                # Проверяем, достиг ли агент выхода
                if x == exit_x and y == exit_y:
                    exit_found = True
                    winning_agent_idx = agent_idx
                    break
                ...
                for dir_idx in order:
                    ...
                    if 0 <= new_x < self.width and 0 <= new_y < self.height and self.maze[new_y, new_x] == 1:
                        ...
                        alpha_value = self.target_distribution[new_y, new_x]

                        # Если альфа -inf, пропускаем эту клетку
                        if not np.isinf(alpha_value):
                            k = alpha_value / (n_visits + 1)
            ...
            if exit_found:
                print(f"✓ Выход найден на итерации {iteration + 1}!")
                break

        if exit_found and winning_agent_idx >= 0:
            self.solution_path = agent_paths[winning_agent_idx]
            self.exit_found = True

        elapsed_time = time.time() - start_time
        return elapsed_time, exit_found

    def run_parallel(self, n_iterations: int, n_processes: int) -> Tuple[float, bool]:
        ...
        # Exit found flag
        shared_exit_found_val = Value(ctypes.c_int, 0)

        # Entry and exit points
        entry_arr = RawArray(ctypes.c_int, 2)
        entry_arr[0], entry_arr[1] = self.entry

        exit_arr = RawArray(ctypes.c_int, 2)
        exit_arr[0], exit_arr[1] = self.exit
        ...
        with Pool(processes=n_processes,
                  ...
                  initargs=(shared_dynamic_arr, shared_target_arr, shared_maze_arr,
                            (height, width), shared_M_arr, shared_exit_found_val,
                            tuple(entry_arr), tuple(exit_arr))) as pool:
        ...
        for local_dynamic, local_M, found_exit, winning_path in results:
            self.dynamic_distribution += local_dynamic
            self.M += local_M

            if found_exit and not exit_found:
                exit_found = True
                self.solution_path = winning_path
                self.exit_found = True
                print(f"✓ Выход найден одним из агентов!")
