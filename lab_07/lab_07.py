...
class MazeNavigationAgent:
    ...
    def __init__(self, maze_path: str, n_agents: int = 100, seed: int = 42):
        ...
        self.history = []
        self.agent_paths = []  # Пути всех агентов
    ...
    def move_agent(self, agent_idx: int) -> Tuple[int, int]:
        ...
        best_alpha = self.alpha[y, x]
        best_x, best_y = x, y
        candidates = []  # Список хороших кандидатов
        ...
        for dir_idx in order:
            ...
            if (0 <= new_x < self.width and 0 <= new_y < self.height and
                ...
                alpha = self.alpha[new_y, new_x]

                # Собираем все направления, которые лучше текущего
                if alpha > best_alpha:
                    if alpha > candidates[0][2] if candidates else float('-inf'):
                        candidates = [(new_x, new_y, alpha)]
                    elif len(candidates) > 0 and abs(alpha - candidates[0][2]) < 1e-6:
                        candidates.append((new_x, new_y, alpha))

                if alpha > best_alpha:
                    best_alpha = alpha
                    best_x, best_y = new_x, new_y

        # Если нашли хорошие направления, выбираем случайно среди них
        if candidates:
            best_x, best_y, _ = candidates[np.random.randint(len(candidates))]
        elif best_x == x and best_y == y:
            # Если застряли, делаем случайный ход в любом проходимом направлении
            for dir_idx in order:
                dx, dy = self.directions[dir_idx]
                new_x, new_y = x + dx, y + dy
                if (0 <= new_x < self.width and 0 <= new_y < self.height and
        ...
        for dir_idx in order:
            ...
            if (0 <= new_x < width and 0 <= new_y < height and
                ...
                if alpha_val > best_alpha:
                    if alpha_val > candidates[0][2] if candidates else float('-inf'):
                        candidates = [(new_x, new_y, alpha_val)]
                    elif len(candidates) > 0 and abs(alpha_val - candidates[0][2]) < 1e-6:
                        candidates.append((new_x, new_y, alpha_val))
...
def plot_maze_scalability(results: dict, save_path: str = 'experiment2_scalability.png'):
    ...
    ax.plot(n_processes, times, 'b-', linewidth=3, marker='o', markersize=8, label='Время выполнения')

    # Идеальная масштабируемость
    t1 = times[0]
    ideal_times = [t1 / n for n in n_processes]
    ax.plot(n_processes, ideal_times, 'r--', linewidth=2, alpha=0.5, label='Идеальная масштабируемость')
    ...
    ax.set_title('Масштабируемость параллельной навигации по лабиринту',
...
if __name__ == "__main__":
    ...
    plot_maze_scalability(scalability_results, save_path='experiment2_scalability.png')
