...
class MazeNavigationAgent:
    ...
    def step(self, n_steps: int = 1):
        """Выполнить n_steps шагов алгоритма (последовательно)"""
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            ...
            'successful_agents': int(self.successful_agents),
            'active_agents': int(np.sum(~self.agents_finished)),
            'success_rate': float(self.successful_agents / self.n_agents * 100)
        }
    ...
    def save_checkpoint(self, iteration: int):
        ...
        if np.any(active_mask):
            ...
            max_alpha = float(np.max(active_alphas))
            avg_alpha = float(np.mean(active_alphas))
        else:
            max_alpha = 0.0
            avg_alpha = 0.0
    ...
    def visualize_progress(self, save_path: str = 'maze_progress.png'):
        ...
        # Выбираем ключевые контрольные точки (используем те, что в истории)
        ...
        for idx, ax in zip(indices, axes_maze):
            ...
            n_active = int(np.sum(~finished))
            # intensity: 1 active -> 0.5 (бордовая), n_agents active -> 1.0 (ярко-красная)
            intensity = 0.5 + 0.5 * (n_active / max(1, self.n_agents))
            ...
            for x, y, is_finished in zip(agents_x, agents_y, finished):
                if 0 <= x < self.width and 0 <= y < self.height:
                    if is_finished:
                        ...
                        # Используем intensity одинаково для всех активных агентов
        ...
        # График прогресса альфа-метрики (БЕЗ успешных агентов)
...
def experiment_1_progress(maze_path: str, n_agents: int = 100, max_processes: int = 24, limit_iterations: int = 10000):
    """
    Эксперимент 1: Для каждого числа процессов 1..max_processes запускаем навигацию агентов,
    останавливаем при достижении любого агента выхода (или по лимиту итераций).
    Время считаем ТОЛЬКО для внутренних шагов (step_parallel / step).
    Сохраняем по каждому n_proc файл outputs/experiment_progress_{n_proc}.png
    """
    print("\n" + "=" * 70)
    print("ЭКСПЕРИМЕНТ 1: Прогресс агентов (1-24 процесса)")
    print("Критерий остановки: первый агент достигает выхода")
    print("=" * 70)
    results = {}
    iterations_per_run = {}

    # контрольные итерации, final добавляем в конце
    control_iters = [0, 100, 500, 1000, 1500]
    ...
    for n_proc in range(1, max_processes + 1):
        ...
        # синхронизируем начальные позиции из shared memory (если есть)
        ...
        # Сохраняем начальную контрольную точку (итерация 0)
        algo.save_checkpoint(0)

        iteration = 0
        # Время меряем только выполнение шагов (внутри loop - step_parallel)
        start_time = time.perf_counter()

        while algo.successful_agents < 1 and iteration < limit_iterations:
            # Выполняем один шаг и мерим только этот внутренний шаг
            algo.step_parallel(1)
            iteration += 1

            # Сохраняем контрольные точки по заданному списку
            if iteration in control_iters:
                # синхронизируем данные из shared memory
                algo.agents_x = np.array(np.frombuffer(algo.shared_agents_x, dtype=np.int32))
                algo.agents_y = np.array(np.frombuffer(algo.shared_agents_y, dtype=np.int32))
                algo.agents_finished = np.array(np.frombuffer(algo.shared_agents_finished, dtype=ctypes.c_bool))
            ...
            # Защита от бесконечного цикла внутри
            if iteration % 1000 == 0:
                print(f"  Итерация {iteration}... успешных: {algo.successful_agents}")

        # остановились - сохраняем последнюю контрольную точку
        # синхронизация
        algo.agents_x = np.array(np.frombuffer(algo.shared_agents_x, dtype=np.int32))
        algo.agents_y = np.array(np.frombuffer(algo.shared_agents_y, dtype=np.int32))
        algo.agents_finished = np.array(np.frombuffer(algo.shared_agents_finished, dtype=ctypes.c_bool))
        ...
        print(f"✓ Время (только шаги) до первого успеха: {elapsed:.4f}с")
        ...
        # Сохраняем визуализацию прогресса (6 контрольных точек: 0,100,500,1000,1500,last)
        # Визуализация сохраняется в outputs/experiment_progress_{n_proc}.png
        save_filename = f"outputs/experiment_progress_{n_proc}.png"
        try:
            algo.visualize_progress(save_path=save_filename)
        except Exception as e:
            print(f"Ошибка при сохранении визуализации для {n_proc} процессов: {e}")
...
if __name__ == "__main__":
    ...
    # Не создаем outputs автоматически - пользователь создает папку сам
    algo_init.visualize_alpha_metric('outputs/alpha_metric.png')

    # ЭКСПЕРИМЕНТ 1: Прогресс для 1..max_proc процессов
    max_proc = min(24, cpu_count())
    print(f"\nМаксимальное количество процессов для тестирования: {max_proc}")

    progress_results, progress_iters = experiment_1_progress(
        maze_path, n_agents=100, max_processes=max_proc, limit_iterations=10000
    )
    ...
    print("=" * 70)
    print("\nСохраненные файлы (ожидаются в outputs/):")
    print("  - alpha_metric.png - визуализация метрики альфа")
    print("  - experiment_progress_{n}.png - прогресс для каждого n процессов (1..N)")
    print("  - experiment_scalability.png - график масштабируемости")
    print("=" * 70)
